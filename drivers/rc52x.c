...
rc52x_result_t PICC_RequestA(rc52x_t *rc52x, picc_t *picc) {
	...
	return PICC_REQA_or_WUPA(rc52x, PICC_CMD_REQA, &picc->atqa, &size);
} // End PICC_RequestA()
...
rc52x_result_t PICC_WakeupA(rc52x_t *rc52x, picc_t *picc) {
	...
	size_t size = sizeof(iso14443a_atqa_t);
	return PICC_REQA_or_WUPA(rc52x, PICC_CMD_WUPA, &picc->atqa, &size);
} //  // End PICC_WakeupA()
...
rc52x_result_t PICC_REQA_or_WUPA(rc52x_t *rc52x, uint8_t command, ///< The command to send - PICC_CMD_REQA or PICC_CMD_WUPA
		...
		size_t *bufferSize	///< Buffer size, at least two uint8_ts. Also number of uint8_ts returned if STATUS_OK.
		) {
	uint8_t validBits;
