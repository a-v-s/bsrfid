...
StatusCode PCD_CalculateCRC(	uint8_t *data,		///< In: Pointer to the data to transfer to the FIFO for CRC calculation.
	...
	rc52x_set_reg8(rc52x, MFRC_REG_CommandReg, PCD_Idle);		// Stop any active command.
	...
	rc52x_set_reg8(rc52x, MFRC_REG_CommandReg, PCD_CalcCRC);		// Start the calculation
	...
	for (uint16_t i = 5000; i > 0; i--) {
		...
		if (n & 0x04) {									// CRCIRq bit set - calculation done
			rc52x_set_reg8(rc52x, MFRC_REG_CommandReg, PCD_Idle);	// Stop calculating CRC for new content in the FIFO.
...
void PCD_Init() {
	...
	rc52x_set_reg8(rc52x, MFRC_REG_TReloadRegH, 0x03);		// Reload timer with 0x3E8 = 1000, ie 25ms before timeout.
	rc52x_set_reg8(rc52x, MFRC_REG_TReloadRegL, 0xE8);
 ...
 */
void PCD_Reset() {
	rc52x_set_reg8(rc52x, MFRC_REG_CommandReg, PCD_SoftReset);	// Issue the SoftReset command.
...
bool PCD_PerformSelfTest() {
	...
	rc52x_set_reg8(rc52x, MFRC_REG_CommandReg, PCD_Mem);		// transfer to internal buffer
	...
	rc52x_set_reg8(rc52x, MFRC_REG_CommandReg, PCD_CalcCRC);
...
StatusCode PCD_CommunicateWithPICC(	uint8_t command,		///< The command to execute. One of the PCD_Command enums.
	...
	rc52x_set_reg8(rc52x, MFRC_REG_FIFOLevelReg, 0x80);				// FlushBuffer = 1, FIFO initialization
	rc52x_set_reg8(rc52x, MFRC_REG_FIFODataReg, sendLen, sendData);	// Write sendData to the FIFO
...
StatusCode PICC_Select(	Uid *uid,			///< Pointer to Uid struct. Normally output, but can also be used to supply a known UID.
	...
	while (!uidComplete) {
		...
		while (!selectDone) {
			...
			rc52x_set_reg8(rc52x, MFRC_REG_BitFramingReg, (rxAlign << 4) + txLastBits);	// RxAlign = BitFramingReg[6..4]. TxLastBits = BitFramingReg[2..0]
 ...
 * @return STATUS_OK on success, STATUS_??? otherwise.
 */
StatusCode PCD_NTAG216_AUTH(uint8_t* passuint16_t, uint8_t pACK[]) //Authenticate with 32bit passuint16_t
{
